...
## Required depth for every future search

The default is a very deep worldwide search, country by country. Aim to discover and check every relevant accessible resource within each country, not just a small set of global platforms or previously productive boards. Source examples above are starting points, never a fixed whitelist. Do not stop because a listing target has been met, one board has returned enough results, or every continent has at least one vacancy.

1. **Plan the full geographic coverage.** Start with the previous run's unfinished countries and source lanes, then expand across all six regions. Keep a country register that explicitly distinguishes researched, partially researched and not yet researched markets. Do not silently omit countries with fewer English-language results or no previous listings. A country with no verified matches still needs its search evidence and limitations recorded.
2. **Discover the country's sources before relying on them.** Build or extend a country-specific inventory: national and local general job boards; technology, automation, finance and shared-services specialist boards; public employment services; recruitment and staffing agencies; employer career sites and their ATS; RPA/platform partner and consultancy directories for employer discovery; publicly accessible professional associations, communities and vacancy posts; and international platforms, remote boards and aggregators. Search for additional local resources in relevant local languages. Mark genuinely unavailable or inapplicable categories with a reason rather than inventing sources or claiming they were searched.
3. **Search independent role and language combinations.** Cover developer, manager, team lead, analyst and product owner separately in each researched country. Apply the discovery lanes above, including titles that omit RPA or automation but have substantive overlap with Kiril's evidence. Use English and relevant local-language titles, synonyms, spelling variants and tools. Candidate language ability and employer requirements remain separate from the language used to discover an advertisement.
4. **Go beyond search-engine snippets.** Use each accessible source's own search, filters and relevant result pages, alongside targeted web searches. Follow leads to original employer postings and search those employers for adjacent relevant vacancies. Expand into newly discovered employers, agencies and specialist sources. Deduplicate reposts and syndication; multiple mirrors do not count as independent discovery coverage. Record login walls, access restrictions, broken searches and inaccessible pages; use legitimate alternative sources without treating blocked resources as checked.
5. **Continue while discovery is productive.** Review further relevant result pages and vary titles, employers, languages and source categories while they yield credible new leads. Record where each search stopped and why. Repeated duplicate or irrelevant results across independent query variants may justify marking a lane as searched to diminishing returns, but never prove that a country has no further jobs. Do not present a time-limited pass as a completed deep search.
6. **Preserve unfinished work visibly.** A bounded run may publish verified vacancies while coverage remains partial. Record every unfinished country, source category, role focus and promising lead, with concrete next queries or sources. Carry these into the next refresh rather than restarting the same few easy sources. Do not remove positively verified vacancies merely because discovery in their country is unfinished.

For each future run, extend `data/search-runs.json` with a structured country/source coverage ledger (preserving previous runs). Each country entry should contain the languages and role focuses attempted, discovered source names and URLs grouped by category, exact queries or source filters, result-page ranges or stopping points, UTC check times, verification outcomes, new unique vacancies found, access blockers, and remaining work. Distinguish source discovery from actual source review. Record only work actually performed; never backfill historical runs as though this deeper protocol had already been followed.

Report geographic breadth and source depth separately from vacancy counts. Identify countries and source categories still missing, explain access limitations, and reserve completeness claims for the specific documented checks. Literal coverage of every resource or vacancy worldwide cannot be guaranteed; the operating requirement is systematic, expanding discovery with auditable gaps, not an unsupported claim of exhaustiveness.
