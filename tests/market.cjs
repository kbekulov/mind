...
(async()=>{
  ...
  try{
    ...
    assert.ok(await page.evaluate(()=>document.querySelector('.research-entry').getBoundingClientRect().top-document.querySelector('.spaces-grid').getBoundingClientRect().bottom>=20),'Space cards and research panel have a visible gap');
    await page.locator('.research-entry').scrollIntoViewIfNeeded();
    await page.screenshot({path:'test-results/home-spacing.png'});
    ...
    for(const hash of ['home','all','board','doc&doc=scroll-test-0','market']){
      ...
      const headings={home:'Make yourself at home.',all:'Everything, in one place.',board:'Let your thinking take shape.','doc&doc=scroll-test-0':'Scroll record 0'};
      if(headings[hash])await page.getByRole('heading',{name:headings[hash],exact:true}).waitFor();
