...
(() => {
  ...
  function render(){
    ...
    const content=section.id==='market'?MindMarket.render():section.id==='religion'?MindReligion.render(topic?.id):`<section class="topic-page"><div class="eyebrow">${topic?section.name.toUpperCase():'RESEARCH LIBRARY'}</div><h1>${topic?.name||section.name}</h1>${topic?'<div class="topic-empty">No research published in this section yet.</div>':`<p class="subtitle">${section.description}</p><div class="topic-grid">${section.topics.map(t=>`<a class="topic-card" href="${href(section,t)}"><h2>${t.name}</h2><span>View section →</span></a>`).join('')}</div>`}</section>`;
